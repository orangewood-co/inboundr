...
function Header({ site, embed }: { site: CareersSite; embed: boolean }) {
  return <header className="border-b border-black/10 bg-[#f4f1ea]/95 backdrop-blur"><div className={`mx-auto flex max-w-6xl items-center justify-between px-5 ${embed ? "py-3" : "py-5"}`}><a href={publicCareersPath(site.organizationPath)} className="flex items-center gap-3 font-display font-semibold text-stone-950">{site.branding.logoUrl ? <img src={site.branding.logoUrl} alt="" className="size-9 rounded-lg object-contain" /> : <span className="flex size-9 items-center justify-center rounded-lg bg-stone-950 text-white"><Building2Icon className="size-4" /></span>}<span>{site.organizationName}</span></a>{!embed && site.website && <a href={site.website} target="_blank" rel="noreferrer" className="flex items-center gap-1.5 text-sm font-medium text-stone-600 hover:text-stone-950">Company site <ExternalLinkIcon className="size-3.5" /></a>}</div></header>
}
...
function CareersListing({ site, jobs, embed, onNavigate }: { site: CareersSite; jobs: CareersJob[]; embed: boolean; onNavigate?: (path: string) => void }) {
  ...
  return <>
    ...
    <main className="bg-[#f4f1ea] text-stone-950">
      <section className={`relative overflow-hidden border-b border-black/10 ${embed ? "py-10" : "py-20 sm:py-28"}`} style={{ backgroundColor: site.branding.primaryColor }}>
        ...
        <div className="relative mx-auto max-w-6xl px-5 text-white"><p className="text-xs font-bold tracking-[.22em] uppercase opacity-75">Careers at {site.organizationName}</p><h1 className={`mt-4 max-w-4xl font-display font-semibold leading-[.95] tracking-tight ${embed ? "text-4xl" : "text-5xl sm:text-7xl"}`}>{site.headline || `Do work that moves things forward.`}</h1>{site.intro && <p className="mt-6 max-w-2xl whitespace-pre-line text-base leading-7 text-white/80 sm:text-lg">{site.intro}</p>}</div>
      </section>
      <section className="mx-auto max-w-6xl px-5 py-12 sm:py-16">
        <div className="flex flex-col justify-between gap-2 sm:flex-row sm:items-end"><div><p className="text-xs font-bold tracking-[.2em] text-stone-500 uppercase">Open opportunities</p><h2 className="mt-2 font-display text-3xl font-semibold">{jobs.length === 1 ? "One role, one big next step" : `${jobs.length} ways to join us`}</h2></div><p className="text-sm text-stone-500">{filtered.length} role{filtered.length === 1 ? "" : "s"} shown</p></div>
        <div className="mt-8 grid gap-3 rounded-2xl border border-black/10 bg-white p-3 shadow-sm sm:grid-cols-[1.4fr_1fr_1fr_1fr]">
          <label className="flex items-center gap-2 rounded-xl bg-stone-100 px-3"><SearchIcon className="size-4 text-stone-400" /><span className="sr-only">Search roles</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search roles" className="min-w-0 flex-1 bg-transparent py-3 text-sm outline-none" /></label>
          {[["Department", department, setDepartment, values("department")], ["Location", location, setLocation, values("location")], ["Type", type, setType, values("employmentType")]].map(([label, current, setter, options]) => <label key={label as string}><span className="sr-only">{label as string}</span><select value={current as string} onChange={(event) => (setter as (value: string) => void)(event.target.value)} className="h-full w-full rounded-xl border-0 bg-stone-100 px-3 py-3 text-sm outline-none focus:ring-2 focus:ring-stone-900"><option value="">All {String(label).toLowerCase()}s</option>{(options as string[]).map((option) => <option key={option}>{option}</option>)}</select></label>)}
        </div>
        <div className="mt-6 divide-y divide-black/10 border-y border-black/10">{filtered.map((job) => { const salary = formatSalary(job); const path = job.seo.canonicalPath || publicCareersPath(site.organizationPath, job.slug); return <a key={job.id} href={`${path}${embed ? "?embed=1" : ""}`} onClick={onNavigate ? (event) => { event.preventDefault(); onNavigate(path) } : undefined} className="group grid gap-4 py-6 transition-[padding] duration-200 hover:px-3 sm:grid-cols-[1fr_auto] sm:items-center"><div><div className="flex flex-wrap items-center gap-2"><h3 className="font-display text-xl font-semibold">{job.title}</h3>{job.deadlineClosed && <span className="rounded-full bg-stone-200 px-2.5 py-1 text-[11px] font-bold tracking-wide text-stone-600 uppercase">Applications closed</span>}</div><div className="mt-2 flex flex-wrap gap-x-5 gap-y-1 text-sm text-stone-500"><span className="flex items-center gap-1.5"><Building2Icon className="size-3.5" />{job.department || "General"}</span><span className="flex items-center gap-1.5"><MapPinIcon className="size-3.5" />{job.location || "Flexible"}</span><span className="flex items-center gap-1.5"><Clock3Icon className="size-3.5" />{job.employmentType || "Role"}</span>{salary && <span className="font-semibold text-stone-700">{salary}</span>}</div></div><span className="flex size-11 items-center justify-center rounded-full border border-black/15 transition-[background-color,color,transform] duration-200 group-hover:translate-x-1 group-hover:bg-stone-950 group-hover:text-white"><ArrowRightIcon className="size-4" /></span></a> })}</div>
...
function ApplicationForm({ site, job }: { site: CareersSite; job: CareersJob }) {
  ...
  return <form onSubmit={submit} className="rounded-3xl border border-black/10 bg-[#f8f6f1] p-5 sm:p-8" noValidate>
