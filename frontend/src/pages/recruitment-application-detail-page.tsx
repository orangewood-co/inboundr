...
export default function RecruitmentApplicationDetailPage() {
  ...
  const emailConflict = handoffConflict?.employeeEmailConflict
  const conflictingEmployee = emailConflict?.exists ? emailConflict : null
  return (
    <RecruitmentShell breadcrumbs={[{ label: "Recruitment", href: "/recruitment" }, { label: "Applicants", href: "/recruitment/applicants" }, { label: candidate?.fullName ?? "Candidate" }]}>
      ...
      <Dialog open={handoffConflict !== null} onOpenChange={(open) => !open && setHandoffConflict(null)}>
        <DialogContent><DialogHeader><DialogTitle>An Employee Already Uses This Email</DialogTitle><DialogDescription>{conflictingEmployee ? `${conflictingEmployee.fullName} (${conflictingEmployee.status}) already has an employee record with ${conflictingEmployee.email}. Continuing may be rejected as a duplicate.` : ""}</DialogDescription></DialogHeader><DialogFooter><DialogClose asChild><Button variant="outline">Cancel</Button></DialogClose>{conflictingEmployee && <Button variant="outline" onClick={() => void navigate({ to: "/employees/$id", params: { id: conflictingEmployee.employeeId } })}>Open Existing Employee</Button>}<Button onClick={() => void continueEmployeeHandoff()}>Continue with Prefill</Button></DialogFooter></DialogContent>
      </Dialog>
    </RecruitmentShell>
  )
}
