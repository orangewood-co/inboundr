...
export function RecruitmentSettingsPage() {
  ...
  const sections = settings.sections ?? { story: true, benefits: true, team: true }
  const patchSection = (key: keyof typeof sections, value: boolean) => patch({ sections: { ...sections, [key]: value } })
  return (
          ...
          <div className="space-y-6">
            ...
            <section className="rounded-2xl border bg-card p-6 shadow-xs">
              <div className="flex items-start justify-between gap-4">
                <div><h2 className="font-semibold">Company Story</h2><p className="text-sm text-muted-foreground">A longer narrative shown below the hero, plus a contact email candidates can write to.</p>{!sections.story && <p className="mt-1 text-xs font-medium text-amber-600">The story section is hidden from the careers page.</p>}</div>
                <Switch checked={sections.story} onCheckedChange={(checked) => patchSection("story", checked)} aria-label="Show the story section on the careers page" />
              </div>
              <div className="mt-5 grid gap-4">
                <div><Label>Contact email</Label><Input className="mt-2" type="email" value={settings.contactEmail ?? ""} onChange={(event) => patch({ contactEmail: event.target.value })} placeholder="talent@company.com" /><p className="mt-1.5 text-xs text-muted-foreground">Shown as a call-to-action next to the open-roles button on the careers page.</p></div>
                <div><Label>Story title</Label><Input className="mt-2" maxLength={120} value={settings.aboutTitle ?? ""} onChange={(event) => patch({ aboutTitle: event.target.value })} placeholder="Careers at your company" /></div>
                <div><Label>Story</Label><textarea className="mt-2 min-h-40 w-full rounded-md border border-input bg-transparent px-3 py-2 text-sm outline-none focus-visible:ring-3 focus-visible:ring-ring/50" maxLength={10000} value={settings.aboutBody ?? ""} onChange={(event) => patch({ aboutBody: event.target.value })} placeholder={"What you're building, how the team works, and why it matters. Separate paragraphs with a blank line."} /><p className="mt-1 text-right text-xs text-muted-foreground">{(settings.aboutBody ?? "").length}/10000</p></div>
              </div>
            </section>
            <section className="rounded-2xl border bg-card p-6 shadow-xs">
              <div className="flex items-start justify-between gap-4"><div><h2 className="font-semibold">Benefits & Perks</h2><p className="text-sm text-muted-foreground">Shown as “Why you’ll love working here” cards on the careers page.</p>{!sections.benefits && <p className="mt-1 text-xs font-medium text-amber-600">The benefits section is hidden from the careers page.</p>}</div><div className="flex shrink-0 items-center gap-3"><Switch checked={sections.benefits} onCheckedChange={(checked) => patchSection("benefits", checked)} aria-label="Show the benefits section on the careers page" /><Button size="sm" variant="outline" onClick={() => patch({ benefits: [...(settings.benefits ?? []), { title: "", description: "" }] })}><PlusIcon /> Add</Button></div></div>
              <div className="mt-5 space-y-3">{(settings.benefits ?? []).length === 0 ? <p className="rounded-xl border border-dashed p-5 text-center text-sm text-muted-foreground">No benefits yet.</p> : (settings.benefits ?? []).map((benefit, index) => <div key={index} className="grid gap-2 sm:grid-cols-[200px_1fr_auto]"><Input aria-label={`Benefit ${index + 1} title`} maxLength={80} value={benefit.title} onChange={(event) => patch({ benefits: (settings.benefits ?? []).map((item, itemIndex) => itemIndex === index ? { ...item, title: event.target.value } : item) })} placeholder="Meaningful work" /><Input aria-label={`Benefit ${index + 1} description`} maxLength={300} value={benefit.description} onChange={(event) => patch({ benefits: (settings.benefits ?? []).map((item, itemIndex) => itemIndex === index ? { ...item, description: event.target.value } : item) })} placeholder="Build infrastructure that powers real products." /><Button size="icon" variant="ghost" aria-label="Remove benefit" onClick={() => patch({ benefits: (settings.benefits ?? []).filter((_, itemIndex) => itemIndex !== index) })}><Trash2Icon /></Button></div>)}</div>
            </section>
            <section className="rounded-2xl border bg-card p-6 shadow-xs">
              <div className="flex items-start justify-between gap-4"><div><h2 className="font-semibold">Team Members</h2><p className="text-sm text-muted-foreground">Introduce the people candidates would work with.</p>{!sections.team && <p className="mt-1 text-xs font-medium text-amber-600">The team section is hidden from the careers page.</p>}</div><div className="flex shrink-0 items-center gap-3"><Switch checked={sections.team} onCheckedChange={(checked) => patchSection("team", checked)} aria-label="Show the team section on the careers page" /><Button size="sm" variant="outline" onClick={() => patch({ teamMembers: [...(settings.teamMembers ?? []), { name: "", role: "" }] })}><PlusIcon /> Add</Button></div></div>
              <div className="mt-5 space-y-3">{(settings.teamMembers ?? []).length === 0 ? <p className="rounded-xl border border-dashed p-5 text-center text-sm text-muted-foreground">No team members yet.</p> : (settings.teamMembers ?? []).map((member, index) => <div key={index} className="grid gap-2 sm:grid-cols-[1fr_1fr_auto]"><Input aria-label={`Team member ${index + 1} name`} maxLength={120} value={member.name} onChange={(event) => patch({ teamMembers: (settings.teamMembers ?? []).map((item, itemIndex) => itemIndex === index ? { ...item, name: event.target.value } : item) })} placeholder="Alexandra Reed" /><Input aria-label={`Team member ${index + 1} role`} maxLength={120} value={member.role} onChange={(event) => patch({ teamMembers: (settings.teamMembers ?? []).map((item, itemIndex) => itemIndex === index ? { ...item, role: event.target.value } : item) })} placeholder="Head of Product" /><Button size="icon" variant="ghost" aria-label="Remove team member" onClick={() => patch({ teamMembers: (settings.teamMembers ?? []).filter((_, itemIndex) => itemIndex !== index) })}><Trash2Icon /></Button></div>)}</div>
            </section>
            <section className="rounded-2xl border bg-card p-6 shadow-xs">
